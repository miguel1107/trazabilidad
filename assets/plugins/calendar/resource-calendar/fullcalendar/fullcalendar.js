...
function ResourceView(element, calendar, viewName) {
	...
	/* buildskeleton contruye la tabla del calendrario horizontal
        /*---------------------------------------------------------------------------------------------------------------------------*/
	...
	function buildSkeleton(maxRowCnt, showNumbers) {
		...
		var headerClass = tm + "-widget-header";
		var contentClass = tm + "-widget-content";
		var i, j, id, resourceName, resourceModcont;
		var table;
		var resources = t.getResources;
		s =
			"<table id='cronograma' class='table-bordered table-striped table-hover' style='width:100%' cellspacing='0'>" +
			"<thead>" +
			"<tr class='fc-first fc-last'><th class='fc-resourceName  fc-widget-header center'style='vertical-align: middle;'>EMPLEADO</th>" +
			"<th class='fc-resourceName  fc-widget-header center' style='vertical-align: middle;'>CONTRATO</th>"
		for (i=0; i<colCnt; i++) {
			s +=
				"<th style='width: 31px;'  class='fc- " + headerClass + "'/>";
		}
		s +=
			"</tr>" +
			"</thead>" +
		...
		for (i=0; i<maxRowCnt; i++) {
			...
			resourceModcont = resources[i]['modcont'];
			
			s +=
				"<tr class='center fc-resourcerow-" + id + "'><td class='fc-resourceName'>" + resourceName + "</td>"
				+"<td class='fc-resourceName'>" + resourceModcont + "</td>";
				//"<tr class='fc-resourcerow-" + modcont + "'><td class='fc-resourceModcont'>" + resourceModcont + "</td>";
			for (j=0; j<colCnt; j++) {
				s +=
					"<td style='width: 31px;' class='fc- " + contentClass + " fc-day" + j + " fc-resource" + id +"'>" + // need fc- for setDayID
					"<div>" +
					(showNumbers ?
...
//			if (date.getDay() == 0 || date.getDay() == 6) cell.addClass('fc-weekend-column');
                        // domingo solo weekend
			if (date.getDay() == 0) cell.addClass('fc-weekend-column');
