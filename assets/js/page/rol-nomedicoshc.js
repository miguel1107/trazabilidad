$(document).on('ready', function(){
	...
	$('#programacion-rol-medico').on('submit', function(e){
		...
		if(dependencia != -1){
			...
			estructura_rol.slideUp("slow", function(){
				...
				var options = {
					type: 'POST',
					url:'index.php?page=rolnomedicoshc&accion=ajaxGetEstadoPrognmhc',
					data: {
	...
	$('body').on('click', '#btnCrearProgramacion', function(e){
		...
		if (dpto != -1 && dependencia != -1 && fecha !=-1){
			...
			var options = {
				type: 'POST',
				url:'index.php?page=rolnomedicoshc&accion=ajaxCrearProgramacion',
				data: {
					'departamento': dpto,
					'dependencia': dependencia,
					'fecha': fecha
				...
				success: function(response){
						...
						if(response.created){
							bootbox.alert(response.msg, function(){
								window.location = 'index.php?page=rolnomedicoshc&accion=programacionesPendientes';
