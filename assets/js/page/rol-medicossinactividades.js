$(document).on('ready', function(){
	...
	$('#programacion-rol-medico').on('submit', function(e){
		...
		if(dependencia != -1){
			...
			estructura_rol.slideUp("slow", function(){
				...
				var options = {
					type: 'POST',
					url:'index.php?page=rolnomedicos&accion=ajaxGetEstadoProg',
					data: {
	...
	$('body').on('click', '#btnCrearProgramacion', function(e){
		...
		if (dpto != -1 && dependencia != -1 && fecha !=-1){
			...
			var options = {
				type: 'POST',
				url:'index.php?page=rolmedicossinactividades&accion=ajaxCrearProgramacion',
				data: {
					'departamento': dpto,
					'dependencia': dependencia,
					'fecha': fecha
				...
				success: function(response){
						...
						if(response.created){
							bootbox.alert(response.msg, function(){
								window.location = 'index.php?page=rolmedicos&accion=programacionesPendientes';
	...
	$('body').on('click', '#btnInsertarComentario', function(e){
		...
		if(comentario.length > 0){
			...
			var options = {
				type: 'POST',
				url:'index.php?page=rolnomedicos&accion=ajaxAgregarComentario',
				data: {
					'prog_id': prog_id,
	...
	$('body').on('click', '#btnInsertarObservacion', function(e){
		...
		if(observacion.length > 0){
			...
			var options = {
				type: 'POST',
				url:'index.php?page=rolnomedicos&accion=ajaxAgregarObservacion',
				data: {
					'prog_id': prog_id,
...
// e.preventDefault();
    bootbox.confirm("ADVERTENCIA: Realmente quiere Corregir esta Observaci&oacute;n?", function(result) { 
    	if(result){
    		var url="index.php?page=rolnomedicos&accion=ajaxActualizarObservacion"
    		$.ajax({   
    			type: "POST",
    			url:url,
    			data:{
	...
	$('body').on('click', '#btnInsertarValidacion', function(e){
		...
		var options = {
			type: 'POST',
			url:'index.php?page=rolnomedicos&accion=ajaxInsertarValidacion',
			data: {
				'prog_id': prog_id,
			...
			success: function(response){
					...
					if (validacion == 4) {
						window.location = "index.php?page=rolnomedicos&accion=programacionesAprobadas";
					};
