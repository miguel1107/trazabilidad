$(document).on('ready', function(){
	...
	var getDependencias = function(departamento){
		...
		var options = {
			type: 'POST',
			url:'index.php?page=rolnomedicos&accion=ajaxGetDependencias',
			data: {
	...
	$('body').on('click', '#btnCrearProgramacion', function(e){
		...
		if (dpto != -1 && dependencia != -1 && fecha !=-1){
			...
			var options = {
				type: 'POST',
				url:'index.php?page=rolnomedicosconactividades&accion=ajaxCrearProgramacion',
				data: {
					'departamento': dpto,
					'dependencia': dependencia,
					'fecha': fecha
				...
				success: function(response){
						...
						if(response.created){
							bootbox.alert(response.msg, function(){
								window.location = 'index.php?page=rolnomedicos&accion=programacionesPendientes';
	...
	$('body').on('click', '#btnInsertarValidacion', function(e){
		...
		var options = {
			...
			success: function(response){
					...
					if (validacion == 4) {
						window.location = "index.php?page=rolmedicos&accion=programacionesAprobadas";
					};
