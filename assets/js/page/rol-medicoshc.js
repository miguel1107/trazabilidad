$(document).on('ready', function(){
	...
	$('#formulario-registro').on('hidden.bs.modal', function(){
		...
		ajax_actividades.fadeOut('fast', function(){
			var options = {
				type: 'POST',
				url:'index.php?page=rolmedicoshc&accion=ajaxGetActividades',
				data: {
					'dependencia': dependencia
				},
				dataType: 'html',
	...
	$('body').on('click', '#btnCrearProgramacion', function(e){
		...
		if (dpto != -1 && dependencia != -1 && fecha !=-1){
			...
			var options = {
				type: 'POST',
				url:'index.php?page=rolmedicoshc&accion=ajaxCrearProgramacion',
				data: {
					'departamento': dpto,
					'dependencia': dependencia,
					'fecha': fecha
				...
				success: function(response){
						...
						if(response.created){
							bootbox.alert(response.msg, function(){
								window.location = 'index.php?page=rolmedicoshc&accion=programacionesPendientes';
	...
	$('body').on('click', '#btnInsertarDetalle', function(e){
	 ...
	 var insertarDetalle = function(emp, detalle, prog_id, dia, mes, anio){
			...
			var options = {
				type: 'POST',
				url:'index.php?page=rolmedicoshc&accion=ajaxInsertarDetalle',
				data: {
					'empleado': emp,
					'detalle': detalle,
					'prog_id': prog_id,
	...
	$('body').on('click', '#btnInsertarObservacion', function(e){
		...
		if(observacion.length > 0){
			...
			var options = {
				type: 'POST',
				url:'index.php?page=rolmedicos&accion=ajaxAgregarObservacion',
				data: {
					'prog_id': prog_id,
	...
	$(document).on('click','#corregir',function(){
    	...
    	// alert(obs);
		// e.preventDefault();
    bootbox.confirm("ADVERTENCIA: Realmente quiere Corregir esta Observaci&oacute;n?", function(result) { 
    	if(result){
    		var url="index.php?page=rolmedicos&accion=ajaxActualizarObservacion"
    		$.ajax({   
    			type: "POST",
    			url:url,
    			data:{
	...
	$('body').on('click', '#btnInsertarValidacion', function(e){
		...
		var options = {
			type: 'POST',
			url:'index.php?page=rolmedicos&accion=ajaxInsertarValidacion',
			data: {
				'prog_id': prog_id,
			...
			success: function(response){
					...
					if (validacion == 4) {
						window.location = "index.php?page=rolmedicoshc&accion=programacionesAprobadas";
					};
	...
	$('body').on('contextmenu', '.detalleProg', function(e){
		...
		bootbox.confirm("ADVERTENCIA: Realmente queire eliminar este registro??", function(result) {    
			if(result){
							var options = {
				type: 'POST',
				url:'index.php?page=rolmedicoshc&accion=ajaxBorrarDetalle',
				data: {
					'detalle': detalle
				},
				dataType: 'html',
