...
/*              var producto = $(this).find('input[name=idprod]').val().split('|');
	...
	var mes = fecha[1];
	var ano = fecha[0];
	var url2="index.php?page=rolesaprobar&accion=obtenerestado"
//alert(url2);	
$.ajax({   
		type: "POST",
		url:url2,
	...
	var url = "index.php?page=rolesturnos&accion=program&d="+d+"&m="+mes+"&a="+ano+"";
    ...
    $(document).ready(function() {
       	$("button[name=guardarestado]").click(function () {
					...
					var url="index.php?page=rolesaprobar&accion=actualizarestado"
