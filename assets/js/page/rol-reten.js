$(document).on('ready', function(){
	...
	$('#programacion-rol-medico').on('submit', function(e){
		...
		if(dependencia != -1){
			...
			estructura_rol.slideUp("slow", function(){
				...
				var options = {
					type: 'POST',
					url:'index.php?page=rolesreten&accion=ajaxGetEstadoProgRtn',
					data: {
	...
	$('body').on('click', '#btnCrearProgramacion', function(e){
		...
		if (dpto != -1 && dependencia != -1 && fecha !=-1){
			...
			var options = {
				type: 'POST',
				url:'index.php?page=rolesreten&accion=ajaxCrearProgramacion',
				data: {
					'departamento': dpto,
					'dependencia': dependencia,
					'fecha': fecha
				...
				success: function(response){
						...
						if(response.created){
							bootbox.alert(response.msg, function(){
								window.location = 'index.php?page=rolesreten&accion=programacionesPendientes';
...
// e.preventDefault();
	...
	$('body').on('click', '#btnInsertarValidacion', function(e){
		...
		var options = {
			type: 'POST',
			url:'index.php?page=rolesreten&accion=ajaxInsertarValidacion',
			data: {
				'prog_id': prog_id,
			...
			success: function(response){
					...
					if (validacion == 4) {
						window.location = "index.php?page=rolesreten&accion=programacionesAprobadas";
					};
